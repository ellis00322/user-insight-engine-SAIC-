...
2. 原文完全没有任何相关线索（哪怕是间接线索）时，才填 null；只要原文出现相关的间接线索，
   ...
   自称/关系词间接推断；age_bracket 可以从"刚毕业""大学生""孩子上大学""退休后"这类
   人生阶段词汇、或明确提到的年龄数字间接推断。年龄性别这两个字段车评原文通常线索很少，
   大部分情况下会是 null，这是正常的，不要为了填满字段而编造
3. 情感强度（intensity）基于原文措辞力度判断，1=轻微，5=强烈
4. 输出必须是合法的 JSON，不得包含任何注释或额外文字"""
...
{{
  "user_profile": {{
    "life_stage": "已婚有娃 | 已婚无娃 | 未婚 | 退休 | null",
    "purchase_motivation": "家用 | 商务接待 | 个人通勤 | 换购升级 | 首次购车 | null",
    "purchase_stage": "已购车 | 意向中 | 对比选购 | null",
    "age_bracket": "18-25 | 26-35 | 36-45 | 46-55 | 56岁以上 | null",
    "gender": "男 | 女 | null"
  }},
  "dimension_sentiments": [
    {{"dimension": "续航 | 智能驾驶 | 内饰 | 充电体验 | 售后服务 | 价格 | 外观 | 空间 | 动力性能 | 其他", "sentiment": "正面 | 负面 | 中性", "intensity": 1}}
  ],
...
ALLOWED_USER_PROFILE_VALUES = {
    ...
    "age_bracket": {"18-25", "26-35", "36-45", "46-55", "56岁以上"},
    "gender": {"男", "女"},
}
...
def normalize_user_profile_enums(data):
    """模型偶尔会把 schema 里 "36-45 | 46-55 | null" 这种带竖线的选项说明整段抄进
    value，而不是选其中一个（不是纯字符串 "null"，normalize_string_nulls 抓不到）。
    不在预设枚举范围内的值一律当无效值处理，归为 None，不让脏值进入后续统计。"""
    up = data.get("user_profile") or {}
    for field, allowed in ALLOWED_USER_PROFILE_VALUES.items():
        if field in up and up[field] is not None and up[field] not in allowed:
            up[field] = None
    return data
...
def main():
    parser = argparse.ArgumentParser(description="批量调用 DashScope API 对清洗后的评论做结构化打标")
