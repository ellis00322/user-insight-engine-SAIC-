...
| [`pipeline/llm_tagging/prompts/feishu_bitable_fields.md`](pipeline/llm_tagging/prompts/feishu_bitable_fields.md) | **当前打标主路径**：飞书多维表格 AI 字段配置（维度提取）+ Python 补全脚本（情感方向 / 用户画像）的分工与 Prompt |
...
| L2 洞察层 | 同 Demo 阶段，规模化后飞书 AI 字段吞吐/配额不够用的部分继续由批量 API 承接 | **飞书多维表格 AI 字段**做维度提取（"评论打标"字段，覆盖面比早期固定枚举更广）+ **Python 批量调用 LLM API 补全**情感方向（按维度）与用户画像（life_stage/purchase_motivation/purchase_stage/gender），结果统一写回同一张飞书多维表格；两部分产出的数据形态一致，产品经理只在飞书里看结果，不接触 API 调用环节，详见 [`pipeline/llm_tagging/README.md`](pipeline/llm_tagging/README.md) |
...
L2 打标方案经历过一次调整，如实记录如下（这个调整过程本身也是"画像准确率引擎"方法论校准气质的体现）：
...
2. **v2（当前主路径）**：改用**飞书多维表格 AI 字段做维度提取**（覆盖面更广，且直接满足命题"借助飞书 AI 工具"的要求），**情感方向与用户画像改由 Python 批量调用 LLM API 补全**——这两个字段飞书这边受 AI 字段吞吐配额限制，规模化跑不完（实测跑到中途配额耗尽），且 Python 侧补的这一步加了 evidence 必须是原文子串的强校验，堵住 v1 暴露的证据造假问题。两部分结果最终合并写回同一张飞书多维表格，下游 Aily / 人群地图 / 回测消费的是同一份数据，感知不到打标环节内部的工具切换。
...
- [ ] Python 补全脚本（情感方向 + 用户画像，替代已废弃的 `tagger.py`，含 evidence 原文子串校验）
